...
async function seed() {
    ...
    const trainingContent = [
        {
            ...
            article: {
                ...
                content: "Ce guide explique comment reconnaitre un message frauduleux a partir de l'expediteur, des liens suspects et du ton employe.\n\nPartagez cet article avec les nouveaux collaborateurs et encouragez-les a signaler tout email douteux.",
            },
            quiz: {
        ...
        },
        {
            ...
            article: {
                ...
                content: "Apprenez comment creer et stocker des mots de passe complexes, quand les renouveler et comment expliquer ces bonnes pratiques aux equipes.",
            },
            quiz: {
        ...
        },
        {
            ...
            article: {
                ...
                content: "Cette fiche rappelle les bons reflexes quand un poste semble infecte ou quand un comportement anormal est detecte sur le reseau.",
            },
            quiz: {
    ...
    for (const content of trainingContent) {
        const savedArticle = await articleRepo.save(articleRepo.create(content.article));
        const savedQuiz = await quizRepo.save(quizRepo.create({
            title: content.quiz.title,
            article: savedArticle,
        }));
        const questionRefs = [];
        for (const questionSeed of content.quiz.questions) {
            const savedQuestion = await questionRepo.save(questionRepo.create({
                text: questionSeed.text,
                quiz: savedQuiz,
            }));
            const savedAnswers = await answerRepo.save(questionSeed.answers.map((answerSeed) => answerRepo.create({
        ...
        console.log(`[seed] Article "${content.article.title}" et quiz "${content.quiz.title}" ajoutes`);
    }
    const quizSimulations = [
        {
            user: learner,
    ...
    for (const simulation of quizSimulations) {
        ...
        if (!quizReference) {
            console.warn(`[seed] Quiz ${simulation.quizSlug} introuvable pour la simulation`);
            continue;
        }
        const savedUserQuiz = await userQuizRepo.save(userQuizRepo.create({
        ...
        const storedAnswers = [];
        for (let i = 0; i < quizReference.questions.length && i < simulation.picks.length; i += 1) {
            ...
            storedAnswers.push(userAnswerRepo.create({
                user: simulation.user,
        ...
        const rawScore = answeredQuestions > 0
            ? (correctAnswers / answeredQuestions) * 100
            : 0;
        savedUserQuiz.score = Math.round(rawScore * 100) / 100;
        await userQuizRepo.save(savedUserQuiz);
        console.log(`[seed] ${simulation.description}: ${correctAnswers}/${answeredQuestions} -> ${savedUserQuiz.score}%`);
    }
